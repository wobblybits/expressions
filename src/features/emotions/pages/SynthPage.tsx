...
const SynthPage: Component = () => {
  ...
  return (
    ...
    <div class="flex flex-col items-center justify-center h-full gap-4">
      ...
      <div class="explainer">
        <h2>The Model</h2>
        <p>The underlying model for the <a href="./">various experiments</a> in this project was created in Python using tagged image datasets[1,2,3] from <a href="https://www.kaggle.com/">Kaggle</a>.</p>
        <p>At first, a straight-forward <code>eigenface</code> approach was taken, using principal component analysis (<code>PCA</code>) to identify the "average" grayscale pixel-representations for each of the tagged emotions. Combining these through weighted addition creates composite images that in some sense correlate to more complex emotional expressions. While simple, a pixel-based approach unfortunately has several drawbacks: 
        </p>
        <ul>
            ...
            <li>The very nature of facial expression is about spatial distortion of features, not pixel-level changes of static locations, which is inherently contradictory to the above needs.</li>
            <li>The PCA components need to be able to "cancel" each other out in superposition, which means many components are in an inverted grayscale. Humans are decidedly not agnostic to this change in polarity.</li>
        </ul>
        <img src="./artifacts/eigenfaces.gif" alt="Pixel-based eigenface approach" />
        <p>In addition to grayscale-value adjustments like gamma correction, a first pass at improving this naive pixel-based approach was made using the <code>dlib</code> face detection library to better normalize the images through rotation, centering, scaling, and masking. Despite noticeable improvements, the fundamental issues with this approach were due to remain. While continuing to try to fine-tune and find workarounds like using <code>sobel</code> filters to detect edges, an epiphany came to use the landmark point coordinates from the face detection library as the data source itself. Instead of pixel-vector components, PCA could now produce coordinate-displacement vectors representing the emotions. This quickly solved almost every issue:</p>
        <ul>
            <li>Coordinates could easily be normalized through <code>affine</code> transformations</li>
